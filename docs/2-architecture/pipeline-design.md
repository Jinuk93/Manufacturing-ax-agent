...
| 파라미터 | 기본값 | 설명 | 결정 시점 |
|---------|--------|------|-----------|
| `PREDICTION_WINDOW_SEC` | 30 | 예측 입력 윈도우 (ADR-007: 30초) | Phase 3 확정 |
| `FORECAST_HORIZON_SEC` | 30 | 예측 범위 (ADR-007: 30초) | Phase 3 확정 |
...
| Neo4j | 온톨로지 그래프 (120노드, 337관계) | 그래프 순회 |
...
| F2 | PREDICTION_WINDOW_SEC | 30 | Phase 3 확정 (ADR-007) |
...
| F5 | LLM_PROVIDER | openai | Phase 3 확정 |
| F5 | LLM_MODEL | gpt-4o-mini | Phase 3 확정 |
...
8. **LLM_PROVIDER:** Phase 3에서 openai / gpt-4o-mini로 확정 ✅ 반영
