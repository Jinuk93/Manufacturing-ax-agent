...
|------|-------------|--------|-----------|------|
| F1, F2 | CNC 센서 데이터 (SCADA) | CNC 가공 | Kaggle CNC Mill (핵심) + Bosch CNC (보조) | ✅ 확보 완료 |
| F2, F3 | 정비/고장 이벤트 로그 | CNC 가공 | 합성 생성 (ADR-002) | ✅ Phase 1 합성 완료 (39건) |
| F3 | 작업 지시 내역 (MES) | CNC 가공 | 합성 생성 (ADR-002) | ✅ Phase 1 합성 완료 (18건) |
| F3 | 부품 재고 현황 (ERP) | CNC 가공 | 합성 생성 (ADR-002) | ✅ Phase 1 합성 완료 (35건) |
| F4 | 설비-고장-부품-매뉴얼 관계 (Ontology) | CNC 가공 | 직접 설계 | Phase 3 |
| F4 | 정비 매뉴얼 텍스트 | CNC 가공 | 미정 | Phase 3 |
...
> **주의:** 아래 내용은 "현재 확보한 데이터"와 "나중에 맞출 목표 표준"을 구분해서 봐야 합니다.
> 실제 확보 컬럼은 Kaggle CNC Mill/Bosch 기준으로, 내부 표준 컬럼과 운영 이벤트 가용성은 Phase 1 EDA 이후 확정합니다.
...
| 구분 | 데이터 항목 | 현재 상태 | 이유 |
|------|-------------|-----------|------|
| 확보 | Kaggle CNC Mill의 전류, 전압, 전력, 위치, 속도, 가속도 | 확보 완료 | F1 전처리와 F2 forecasting의 핵심 입력 |
| 확보 | `tool_condition`, `machining_process` 등 실험 맥락 컬럼 | 확보 완료 | 공정 단계와 공구 상태를 함께 해석하기 위함 |
| 보조 | Bosch CNC의 진동 데이터 | 확보 완료 | Kaggle에 없는 진동 계열 보조 참고 |
| ✅ 확보 | `timestamp` | Phase 1 합성 완료 (sequence × 100ms → datetime) | 시계열 정렬 및 윈도우 생성 기준 |
| ✅ 확보 | `equipment_id` | Phase 1 매핑 완료 (exp→CNC-001~003) | 설비 단위 학습 및 타 시스템 조인 키 |
| 미보유 | 온도 센서 | 현 핵심 데이터셋에는 없음 | Phase 2에서 전류/전력 기반 과열 징후 감지 검증 |
| ✅ 확보 | 정비/고장 이벤트 | Phase 1 합성 완료 (39건) | 이상 신호의 의미 해석 근거 |

#### 4.1.2. 목표 내부 표준 형식 기준

| 구분 | 데이터 항목 | 필요 수준 | 이유 |
|------|-------------|-----------|------|
| 필수 | `timestamp` | 반드시 필요 | 시계열 정렬 및 윈도우 생성 기준 |
| 필수 | `equipment_id` | 반드시 필요 | 설비 단위 학습 및 타 시스템 조인 키 |
| 필수 | 전류, 전력, 속도, 위치 등 핵심 센서 시계열 | 반드시 필요 | 이상 패턴 및 미래 위험 신호 탐지 |
| 중요 | 진동, 온도 등 보강 센서 | 매우 권장 | 설명력과 고장 해석력 향상 |
| 중요 | 운전 상태(run/idle/stop) | 매우 권장 | 정상 패턴을 운전 모드별로 분리하기 위함 |
...
### 6.1. 핵심 실데이터: Kaggle CNC Mill Tool Wear

#### 6.1.1. 데이터 개요

- **출처:** Kaggle CNC Mill Tool Wear
- **대상 설비:** CNC 기계 가공 설비
- **용도:** F1 전처리, F2 이상탐지 및 Forecasting
- **현재 역할:** Phase 1 EDA의 주력 데이터셋

#### 6.1.2. 우선 확인할 항목

| 항목 | 용도 | 비고 |
|------|------|------|
| X/Y/S축 전류, 전력, 속도 | 이상 탐지, 예측 | 핵심 센서 후보 |
| 위치/가속도 계열 | 공정 상태 해석 | 핵심 보조 컬럼 |
| `tool_condition`, `machining_process` | 공정 맥락 파악 | 라벨/단계 정보 |
| `sequence` | 시계열 순서 기준 | `timestamp` 합성의 기초 |
| `experiment` 단위 구분 | 설비 단위 매핑 기준 | `equipment_id` 매핑의 기초 |

> **주의:** 위 항목은 현재 확보 파일 기준의 사전 정리입니다.
> 실제 유효 컬럼, 상수 컬럼 제거 여부, `timestamp`/`equipment_id` 합성 규칙은 Phase 1에서 확정합니다.

#### 6.1.3. 확보 방법

- Kaggle CNC Mill 원본 CSV를 기준으로 실험별 컬럼, 샘플링 간격, 결측률 점검
- `sequence × 100ms -> datetime`, `experiment -> CNC-ID` 매핑 규칙 검토
- 상수 컬럼, 0값 고정 컬럼, 실질 유효 컬럼 수를 먼저 확정

#### 6.1.4. 참고 데이터: Bosch CNC

- ~~KAMP~~ — 카탈로그/메타데이터만 보유, Phase 1에서 삭제
- Bosch CNC는 진동 계열 보조 데이터로 보관 유지 (3축 가속도 2kHz, 3대 기계, 1,702 H5 파일)
- Kaggle과 조인 불가(별도 독립 데이터셋), Phase 2+에서 활용 여부 판단
...
| start_time | 작업 시작 시각 | 기준 센서 타임라인 기준 |
| end_time | 작업 종료 시각 | 진행중이면 null 가능 |

#### 6.2.2. 설계 원칙

- 기준 센서 데이터의 `timestamp`와 정렬 가능해야 함
...
> **참고:** 이 시나리오는 Phase 1에서 IT 합성 스키마로 구체화되었습니다. 상세는 [it-data-synthesis-schema.md](../1-data-exploration/it-data-synthesis-schema.md) 참조.

### 6.6. 데이터 간 연결 관계

```text
Kaggle CNC Mill 센서 (실데이터)
	|
...
1. Kaggle CNC Mill 핵심 센서 데이터
2. 센서 컬럼, 주기, 결측치 현황, `sequence`/`experiment` 기반 표준화 가능성
...
Phase 1 EDA가 완료되어 아래가 확정되었습니다:
- 유효 42컬럼 확정, timestamp/equipment_id 합성 완료
- IT 데이터(MES 18건, Maintenance 39건, ERP 35건) 합성 완료
- 상세: [data-gap-analysis.md](../1-data-exploration/data-gap-analysis.md), [it-data-synthesis-schema.md](../1-data-exploration/it-data-synthesis-schema.md)

온톨로지 설계 방향은 Phase 3에서 구체화합니다.
...
|-------------|-------------|----------------|----------------|-----------|-------------|
| 핵심 센서 데이터 (Kaggle CNC Mill) | 시계열 센서 데이터 | experiment, sequence, current/power/velocity/position 계열 | Phase 1에서 experiment -> 설비 ID 매핑 | PostgreSQL TimescaleDB | F1 전처리, F2 이상탐지, forecasting |
...
flowchart LR
	subgraph Sources[원천 데이터]
		SCADA[Kaggle CNC Mill 센서\n- experiment / sequence\n- current/power/velocity\n- timestamp는 Phase 1 합성]
		MES[MES 작업지시\n- work_order_id\n- equipment_id\n- start_time/end_time]
		ERP[ERP 재고\n- part_id\n- equipment_id\n- snapshot_time]
		LOG[정비 이벤트 로그\n- maintenance_time\n- failure_code]
		DOC[정비 매뉴얼 문서\n- manual_id\n- equipment_type\n- part_id]
