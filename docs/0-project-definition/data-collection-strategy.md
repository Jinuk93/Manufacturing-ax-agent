...
| 미보유 | `timestamp` | Phase 1에서 합성 필요 | 시계열 정렬 및 윈도우 생성 기준 |
| 미보유 | `equipment_id` | Phase 1에서 매핑 필요 | 설비 단위 학습 및 타 시스템 조인 키 |
| 미보유 | 온도 센서 | 현 핵심 데이터셋에는 없음 | 과열은 전류/전력 기반 대체 설계 필요 |
| 미보유 | 실제 정비/고장 이벤트 | 추후 합성 필요 | 이상 신호의 의미 해석 근거 |
...
실제 확보 데이터셋의 컬럼과 시간축 특성은 Phase 1 EDA 이후 확정 반영하는 방식이 가장 현실적입니다.
